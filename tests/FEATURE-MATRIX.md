...
| Paywall | CTA offers only the levels selected for the post (falling back to level rules when none are selected); guest teaser/CTA and no paid-body leakage through singular, archive/search, custom excerpt, full/excerpt feed, or REST raw/rendered representations; global preview fallback; entitled-member and post-editor full content | `npm test` |
| Accounts | A membership bought with an email that already has an account is told to log in (same-site redirect back to the content, off-site return URL refused) and is never auto-logged-in; no prompt for unlinked buyers, plain donations, or an already logged-in buyer | `npm test` |
...
| Gateway audit notes | Every `payment_note` projector (Stripe intent, Stripe invoice, PayPal order, PayPal IPN, refund) keeps only its named operational keys and drops `client_secret`, API keys, payer/customer contact and address data, merchant metadata, raw payloads, nested structures and unknown future fields; Stripe confirmation replay still reads `status` and PayPal settled-order replay still reads the order id from the minimized notes | `npm test` |
...
| Gateway replay safety | Stripe confirmation resolved to a local transaction before any lease or provider call, subscription binding validated, settled result replayed from rows, two confirmations of one intent from two addresses serialized, failed confirmation released and retryable, with a payment left unbound by the upgrade refused in the browser at no provider cost and still settled by its webhook; duplicate Stripe event neither re-fetched nor re-applied while an in-flight one stays retryable (409); duplicate VERIFIED PayPal IPN applied once and distinct notifications never merged; two PayPal orders for one donation cannot both capture | `npm test` |
...
| Payment transition ownership | Every successful provider event settles through one atomic transition service: a genuine initial Stripe subscription invoice settles the transaction, supporter, subscription and entitlement the browser never confirmed; a browser-confirmed first payment is not settled again by its invoice; a renewal creates exactly one paid transaction and dispatches the canonical transition once, with the same invoice under a new event id, a redelivered event, and an interrupted delivery's stranded row all adding no row and no repeated hook, email, activity or entitlement work; an invoice being recorded by another worker answers 409 and stays redeliverable; a verified PayPal IPN and a PayPal capture settle one donation exactly once between them; and no invoice or notification can move a refunded payment back to paid, nor can a refund that lands during a browser confirmation leave the subscription activated or the entitlement re-opened, nor can a renewal redelivered after a cancellation restart the agreement; a refund committing before the announcement is what gets announced, so no paid email, activity entry or entitlement follows it | `npm test` |
...
- [ ] Closing the browser immediately after paying the first subscription invoice still leaves the supporter, transaction, subscription and entitlement settled once the webhook arrives, with exactly one donor receipt.
- [ ] A sandbox renewal produces one new transaction, one receipt and one renewal activity entry even when the invoice is redelivered or announced under a second event id.
...
- [ ] A supporter-only delegated viewer sees the full supporter detail screen with no transaction ID, card, mode, or gateway-link column; the same page for a payment-capable admin shows all of them and the refund action still works.
